...
class Maincontroller
{
	...
	public function getData()
	{
		...
		$select_query = "SELECT DAY(`CheckInDate`) as 'check_in_date',DAY(`CheckOutDate`) as 'check_out_date' FROM `booking_master` WHERE HouseId=" . $this->getRequest()->getParam('homeId');
	...
	{
		$target_dir    = "Uploads/CustomerKYC/";
	...
	public function fetchNearBy($place, $guests)
	{
		$query = "SELECT * FROM `near_by_places` JOIN near_by_places_transaction ON near_by_places_transaction.NearPlaceId = near_by_places.NearPlaceId WHERE NearPlaceName LIKE '%$place%'";
		$sql   = mysqli_query($this->con, $query);
		...
		if(mysqli_num_rows($sql) == 0){
			...
			while ($result = mysqli_fetch_assoc($sql)) 
			{
				$hId = $result['HouseId'];
				// echo $hId;
				$query2 = "SELECT * FROM house_master WHERE NoofAllowedGuest = $guests AND $hId";
				$sql2 = mysqli_query($this->con, $query2);
				return $sql2;
			}	
		}	
	...
	public function nearHouse($nearBy)
	{
		$query = "SELECT * FROM house_master JOIN area_master ON area_master.AreaId = house_master.AreaId WHERE area_master.AreaName LIKE '$nearBy%'";
		$sql   = mysqli_query($this->con, $query);
		return $sql;
	}

	public function dateCheck($checkIn,$checkOut,$homeId)
	{
		$query = "SELECT * FROM `booking_master` WHERE HouseId=$homeId AND CheckInDate >= '$checkIn' AND CheckInDate <= '$checkOut' ";
		$sql = mysqli_query($this->con, $query);
		return $sql;
	...
	{
		extract($_POST);
		$query = "INSERT INTO `booking_master`(`CustId`, `HouseId`, `CheckInDate`, `CheckOutDate`, `AdvancePayment`, `DuePayment`, `TranactionId`, `TotalNoOfGuest`) VALUES ($custId,$houseId,'$checkIn','$checkOut',$amount,$amount,'$razorpay_payment_id',$guests)";
		$sql = mysqli_query($this->con,$query);
		return TRUE;
	}

	public function displayBooking($custId)
	{
		$query = "SELECT * FROM house_master JOIN booking_master ON booking_master.HouseId = house_master.HouseId JOIN area_master ON area_master.AreaId = house_master.AreaId JOIN city_master ON area_master.CityId = city_master.CityId  WHERE booking_master.CustId = $custId";
		$sql = mysqli_query($this->con,$query);
		return $sql;

	}

	public function displayStatus($bookingId)
	{
		$query = "SELECT * FROM house_master JOIN booking_master ON booking_master.HouseId = house_master.HouseId JOIN area_master ON area_master.AreaId = house_master.AreaId JOIN city_master ON area_master.CityId = city_master.CityId  WHERE booking_master.bookingId = $bookingId";
		$sql = mysqli_query($this->con,$query);
		$result = mysqli_fetch_assoc($sql);
		return $result;
