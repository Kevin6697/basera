<?php
	session_start();
		...
		$result=$host->displayForCheckIn();
	if(isset($_POST['submit']))
	{
		$status=$host->addOrder();
						...
						<table class="htlfndr-personal-info-table">
									...
									if(mysqli_num_rows($result)>0)
									{
										while($data=mysqli_fetch_assoc($result))
										{
											...
											echo"<tr>
												...
												<input type=hidden name=BookingId value=".$data['BookingId']."><input type=hidden name=CustEmail value=".$data['CustEmail']."><input type=submit class=btn-primary name=submit value=Check-In></td>
												</form>
												</tr>";	
											echo"<tr>
												<td></td>
								...
								<script>
										$(document).ready(function() {
												swal({
														title:"info",
														text: "Not Check-In(s) Found For Today!",
														type: "info"
													},
													  function(){
